...
const PRIORITY_CONFIG = {
  ...
  medium: { label: 'Moyen',   color: 'bg-blue-100 text-blue-600 dark:bg-blue-900/30' },
  ...
  return (
    <AppLayout>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            <select value={data.status} onChange={e => setData('status', e.target.value)}
              className="px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700
                text-gray-900 dark:text-white text-sm outline-none focus:border-[#2E86C1]">
            ...
            <div className="flex gap-2">
              <button onClick={applyFilters}
                className="flex-1 bg-[#2E86C1] text-white text-sm rounded-lg px-3 py-2 hover:bg-[#1A3A5C] transition-colors">
                Filtrer
              </button>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
          {tickets.data && tickets.data.length > 0 ? (
                        ...
                        <tr key={ticket.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                          <td className="px-4 py-3">
                            <Link href={route('support.tickets.show', ticket.id)}
                              className="font-mono text-sm text-[#2E86C1] hover:underline">
                              {ticket.ticket_number}
                            </Link>
                          </td>
                          <td className="px-4 py-3">
                            <Link href={route('support.tickets.show', ticket.id)}
                              className="text-sm text-gray-900 dark:text-white hover:text-[#2E86C1] line-clamp-1 max-w-xs block">
                              {ticket.subject}
            ...
            <div className="text-center py-16">
              ...
              <Link href={route('support.tickets.create')}
                className="inline-flex items-center gap-2 bg-[#2E86C1] hover:bg-[#1A3A5C] text-white font-semibold px-6 py-3 rounded-xl transition-colors">
      ...
      <div className="fixed bottom-6 right-6 sm:hidden">
        <Link href={route('support.tickets.create')}
          className="flex items-center justify-center w-14 h-14 bg-[#2E86C1] hover:bg-[#1A3A5C] text-white rounded-full shadow-xl transition-colors text-2xl">
          +
        </Link>
      </div>
    </AppLayout>
