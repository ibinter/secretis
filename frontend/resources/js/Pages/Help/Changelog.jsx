/**
 * SECRETIS ERP — Pages/Help/Changelog.jsx
 * Changelog public — timeline verticale des versions SECRETIS
 */

import React, { useState } from 'react'
import { Head, Link } from '@inertiajs/react'

// ─── Données statiques (fallback si Inertia ne les fournit pas) ───────────────
const DEFAULT_VERSIONS = [
  {
    version: '2.0.0', codename: 'Vague 5', released_at: '2026-01-15',
    highlight: 'Performance, observabilité et déploiement enterprise',
    entries: [
      { type: 'new',      text: 'Tests de charge k6 — scénarios multi-tenant (10 000 users simultanés)' },
  ...
  {
    version: '1.5.0', codename: 'Vague 4', released_at: '2025-10-01',
    highlight: 'Application mobile, CI/CD et accessibilité WCAG 2.1 AA',
    entries: [
      { type: 'new',      text: 'Application mobile React Native (iOS & Android) — 8 modules ERP offline-first' },
      { type: 'new',      text: 'Tests E2E Playwright — couverture de 47 scénarios critiques' },
      { type: 'new',      text: 'Stack Docker production : Nginx, PHP-FPM optimisé, Redis cluster' },
      { type: 'new',      text: 'Pipeline CI/CD GitHub Actions — lint, tests, build, déploiement blue/green' },
      { type: 'improved', text: 'Conformité WCAG 2.1 AA — navigation clavier, ARIA, contrastes certifiés' },
      { type: 'improved', text: 'Performance SQL — 23 index ajoutés, temps médian réduit de 40 %' },
      { type: 'fixed',    text: 'Pagination infinie GED corrigée (> 1 000 documents)' },
      { type: 'fixed',    text: 'Conflit de timezone lors de réunions planifiées hors zone OHADA' },
    ],
  },
  {
    version: '1.0.0', codename: 'Vague 3', released_at: '2025-07-01',
    highlight: 'Console SuperAdmin, CRM Prospects et Académie',
    entries: [
      { type: 'new',      text: 'Console SuperAdmin — gestion organisations, licences, feature flags, annonces' },
      { type: 'new',      text: 'CRM Prospects intégré — pipeline Kanban, scoring automatique, import CSV/Excel' },
      ...
      { type: 'new',      text: 'Journal d\'audit complet — toutes actions tracées, export JSON/CSV' },
      { type: 'improved', text: 'Refonte UI complète en dark mode natif (TailwindCSS v3.4)' },
      { type: 'improved', text: 'Onboarding wizard 6 étapes avec emails de relance automatisés J+1/J+3/J+7' },
    ],
  },
  {
    version: '0.9.0', codename: 'Version initiale', released_at: '2025-04-01',
    ...
    entries: [
      ...
      { type: 'new', text: 'Authentification MFA (TOTP + SMS) avec sessions multi-appareils sécurisées' },
      { type: 'new', text: 'SARA v1 — assistant IA basé sur GPT-4o, intégré à tous les modules' },
      { type: 'new', text: 'Paiements OHADA — 11 familles de dépenses, export SYSCOHADA certifié' },
      { type: 'new', text: 'Architecture multi-tenant avec isolation complète des données par organisation' },
      { type: 'new', text: 'Support multilingue : Français, Anglais, Arabe, Portugais' },
    ],
  },
]
...
// ─── Composant principal ──────────────────────────────────────────────────────
export default function Changelog({ versions = DEFAULT_VERSIONS }) {
  const [filter, setFilter] = useState('all') // all | new | improved | fixed

  const filtered = (entries) => filter === 'all' ? entries : entries.filter(e => e.type === filter)
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-3 flex items-center gap-2 overflow-x-auto">
          <span className="text-xs font-medium text-gray-400 whitespace-nowrap mr-1">Filtrer :</span>
          {[
            { key: 'all',      label: 'Tout voir',    icon: '📋' },
            { key: 'new',      label: 'Nouveautés',   icon: '✨' },
            { key: 'improved', label: 'Améliorations', icon: '⚡' },
            { key: 'fixed',    label: 'Corrections',  icon: '🔧' },
          ].map(f => (
            <button
              key={f.key}
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        <div className="relative">
          ...
          <div className="space-y-12">
            {versions.map((version, vi) => {
              const entries = filtered(version.entries)
              if (entries.length === 0 && filter !== 'all') return null
                ...
                <div key={version.version} className="relative flex gap-6">
                  ...
                  <div className="flex-1 pb-2">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <h2 className="text-xl font-bold text-gray-900 dark:text-white">v{version.version}</h2>
                          <span className="px-2.5 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full text-xs font-medium">{version.codename}</span>
                          {isLatest && (
                            <span className="px-2.5 py-0.5 bg-[#1E8449] text-white rounded-full text-xs font-semibold animate-pulse">
                              Dernière version
                            </span>
                          )}
                        </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                      {(filter === 'all' ? version.entries : entries).map((entry, ei) => (
                        <div key={ei} className={`flex items-start gap-3 px-5 py-3.5 ${ei > 0 ? 'border-t border-gray-100 dark:border-gray-700' : ''}`}>
                          <span className="flex-shrink-0 mt-0.5 text-lg" aria-hidden="true">{ENTRY_ICON[entry.type]}</span>
                          <div className="flex-1 flex flex-wrap items-start gap-2 min-w-0">
                            <span className={`flex-shrink-0 px-2 py-0.5 rounded-full text-xs font-semibold leading-none ${BADGE[entry.type].className}`}>
        ...
        <div className="mt-16 text-center border-t border-gray-200 dark:border-gray-700 pt-8">
          ...
          <p className="text-gray-300 dark:text-gray-600 text-xs mt-2">SECRETIS ERP — Conçu pour l'Afrique, certifié OHADA</p>
        </div>
      </div>
    </div>
  )
