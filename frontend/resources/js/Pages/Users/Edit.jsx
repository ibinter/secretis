...
export default function UserEdit({ user = {}, departments = [], available_roles = [] }) {
    ...
    <AuthLayout>
      ...
      <div className="max-w-2xl mx-auto py-8 px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-6">
          ...
          <span>/</span>
          <Link href={route('users.show', user.id)} className="hover:text-indigo-600 dark:hover:text-indigo-400 transition">{user.name}</Link>
          <span>/</span>
          <span className="text-gray-700 dark:text-gray-300 font-medium">Modifier</span>
        </div>

        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          <h1 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2 mb-6">
            <Save size={18} className="text-indigo-500" /> Modifier {user.name}
          </h1>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <Field label="Rôle principal" error={errors.role}>
            ...
            <div className="border-t border-gray-100 dark:border-gray-700 pt-4">
              <p className="text-xs text-gray-400 mb-3">Laisser vide pour conserver le mot de passe actuel</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="Confirmer" error={errors.password_confirmation}>
                  <input type="password" value={data.password_confirmation}
                    ...
                    placeholder="Répéter" className={inputCls} />
                </Field>
              </div>
            </div>
