@php
    /**
     * Facture — SECRETIS ERP
     * Appelée par App\Services\AccountingService::generateInvoicePdf()
     * Variables : $invoice (App\Models\Invoice), $client (AccountingClient), $org (Organization), $items (Collection<InvoiceItem>)
     */
...
<table class="header">
    <tr>
        ...
        <td>
            ...
            <div class="doc-status">{{ $statuts[$invoice->status ?? ''] ?? ($invoice->status ?? '') }}</div>
        </td>
    </tr>
</table>
    ...
    <tr>
        <td style="padding-right:6px;">
            <div class="box">
                ...
                <div class="box-name">{{ $invoice->title ?? '—' }}</div>
            </div>
        </td>
...
<table class="dates">
    <tr>
        ...
        <td>{{ !empty($invoice->issue_date) ? \Carbon\Carbon::parse($invoice->issue_date)->format('d/m/Y') : '—' }}</td>
        <td class="lbl">Échéance</td>
        <td>{{ !empty($invoice->due_date) ? \Carbon\Carbon::parse($invoice->due_date)->format('d/m/Y') : '—' }}</td>
    ...
    <tr>
        <td style="width:52%; padding-right:12px;">
            ...
            <div class="notes">
                ...
                @else
                    Règlement à réception de facture @if(!empty($invoice->due_date)), au plus tard le
                    {{ \Carbon\Carbon::parse($invoice->due_date)->format('d/m/Y') }}@endif.
                    Tout retard de paiement pourra donner lieu à des pénalités de retard.
                @endif
                @if(!empty($invoice->payment_method))
                    <br>Mode de règlement : {{ $reglements[$invoice->payment_method] ?? $invoice->payment_method }}.
                @endif
            </div>
        </td>
        <td style="width:48%;">
            <table class="rec">
                <tr>
                    ...
                    <td class="num">{{ $fmt($invoice->subtotal ?? 0) }} {{ $devise }}</td>
                </tr>
                @if((float) ($invoice->discount_amount ?? 0) > 0)
                    <tr>
                        <td class="lbl">Remise</td>
                        <td class="num">- {{ $fmt($invoice->discount_amount) }} {{ $devise }}</td>
                    </tr>
                @endif
                <tr>
                    <td class="lbl">TVA ({{ rtrim(rtrim(number_format((float) ($invoice->tax_rate ?? 0), 2, ',', ' '), '0'), ',') }} %)</td>
                    <td class="num">{{ $fmt($invoice->tax_amount ?? 0) }} {{ $devise }}</td>
                </tr>
                <tr class="grand">
                    <td>TOTAL TTC</td>
                    <td class="num">{{ $fmt($invoice->total ?? 0) }} {{ $devise }}</td>
                </tr>
                @if((float) ($invoice->paid_amount ?? 0) > 0)
                    <tr>
                        <td class="lbl">Déjà réglé</td>
                        <td class="num">{{ $fmt($invoice->paid_amount) }} {{ $devise }}</td>
                    </tr>
                    <tr>
                        <td class="lbl"><strong>Reste à payer</strong></td>
                        <td class="num"><strong>{{ $fmt($invoice->balance_due ?? 0) }} {{ $devise }}</strong></td>
                    </tr>
                @endif
                @if(!empty($invoice->payment_date))
                    <tr>
                        <td class="lbl">Date de règlement</td>
                        <td class="num">{{ \Carbon\Carbon::parse($invoice->payment_date)->format('d/m/Y') }}</td>
                    </tr>
                @endif
            </table>
        </td>
...
<div class="footer">
    ...
    Facture établie conformément au référentiel SYSCOHADA révisé. Montants exprimés en {{ $devise }}.<br>
